...
- **First URL** — `https://api.anthropic.com/v1/design/h/dcS44xk5UoAtMuAacfWneQ?open_file=Pi+Desktop.html`. Returned **HTTP 404** from this environment, on both initial fetch and a confirming retry (`WebFetch` plus direct `curl` across six URL variants on both `api.anthropic.com` and `claude.ai`). The endpoint is Anthropic's internal authenticated design-preview host and was not reachable from the build environment. This is explicitly flagged here rather than silently substituted, per the brief's instruction to "say so — never silently substitute."
...
- **Third URL** — `https://api.anthropic.com/v1/design/h/WG1KN8u9Y1y76707V7OMUA?open_file=Pi+Desktop.html`. Supplied later for cross-verification. Returned a gzipped tarball that is **byte-identical** to the second-URL bundle (`diff -r` exit 0 across every file). No content has shifted — the implementation continues to track `prd/design-bundle/`.
